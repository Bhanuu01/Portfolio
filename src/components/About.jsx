...
import { personalInfo, education, publications } from '../data/portfolioData.jsx';
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {education.map((edu, index) => (
            ...
            >
              ...
              {edu.location && (
                <p className="text-sm text-accent-1 font-mono mt-1">{edu.location}</p>
              )}
              {edu.coursework && (
                <p className="text-sm text-text-secondary mt-3 leading-relaxed">{edu.coursework}</p>
              )}
            </motion.div>
          ))}
        </div>

        <h3 className="text-2xl font-semibold text-center text-accent-1 mt-16 mb-8 font-mono">Publications</h3>
        <div className="max-w-4xl mx-auto">
          {publications.map((pub, index) => (
            <motion.div
              key={index}
              className="bg-primary-bg p-6 rounded-lg shadow-xl hover:shadow-accent-1/20 transition-shadow duration-300 mb-6"
              variants={cardVariants}
              initial="hidden"
              whileInView="visible"
              custom={index}
              viewport={{ once: true, amount: 0.2 }}
            >
              <p className="text-lg font-semibold text-text-primary">{pub.title}</p>
              <p className="text-sm text-text-secondary font-mono mt-1">{pub.venue} {pub.year ? `• ${pub.year}` : ''}</p>
            </motion.div>
          ))}
        </div>
      </div>
